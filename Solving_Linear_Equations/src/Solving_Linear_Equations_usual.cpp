...
const double Solving_Linear_Equations_interface::ti_plus = 0.01;
const double Solving_Linear_Equations_interface::ti_minus = -0.01;

Solving_Linear_Equations_usual::Solving_Linear_Equations_usual(Matrix A, std::vector<double> x, std::vector<double> b)
	: A(A), N(A.size()), x(x), b(b)
{
	norm_denominator = find_norm(b);
}

double Solving_Linear_Equations_usual::multiply_row_by_column(const std::vector<double>& row, const std::vector<double>& column) const
{
	double result = 0.0;
	for (int i = 0; i < N; ++i) {
...
double Solving_Linear_Equations_usual::find_norm(const std::vector<double>& row) const
{
	double result = 0.0;
